...
converted to WebP. The source hashes below identify the generated originals;
the public repository contains only the optimized assets needed to run the
experience.
...
The generated originals and chroma-key intermediate are not required to build
or run the project and are not published in this repository.
